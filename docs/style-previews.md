...
`illustration`) and `prompt` (1–12000 characters). It returns PNG bytes (`image/png`) with
...
and deletes the local cached preview in a finally block. A cleanup failure is
logged and does not discard a successfully generated preview. It does not reference
