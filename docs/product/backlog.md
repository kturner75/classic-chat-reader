...
12. ~~**`BL-043.1` / `.12` prod auth gate + Secure cookies**~~ **Done** (PR #152; live `/api/auth/status` re-probe after deploy). ~~**`BL-043.2` Google OAuth email auto-link consent**~~ **Done this PR** (password re-auth before link; sessions deleted on link). Remaining **`BL-043` FERPA P0** (`.3` DPA, `.4` invite lifecycle, `.5` access-log writers, `.6` account delete + retention, `.7` server chat-export API).
