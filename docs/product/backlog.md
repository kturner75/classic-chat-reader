...
12. ~~**`BL-043.1` / `.12` prod auth gate + Secure cookies**~~ **Done this PR** (code fail-closed). Remaining **`BL-043` FERPA P0** (`.2` OAuth link, `.3` DPA, `.4` invite lifecycle, `.5` access-log writers, `.6` account delete + retention, `.7` server chat-export API). Live `/api/auth/status` re-probe after deploy.
