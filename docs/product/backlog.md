...
- 2026-08-24: Added `BL-079` (Operator Art Studio; originally `BL-073` in #160). Local separate app in `kturner75/ccr-studio` for review / cherry-pick regen / prompt-edit multi-sample / winner-only transfer. Not product UI / not `/admin`. `BL-065.3` stays bulk/unattended. Docs only; **Proposed**.
- 2026-08-24: Refined `BL-079` transfer/picker constraints: deletions must reach prod (tombstone or reviewed-roster replacement); regenerated metadata uses overwrite so CDN `?v=` changes; book picker type-ahead / ordered / keyboard listbox / no-results. Docs only.
- 2026-08-27: Folded leftover #160 Codex P1s into `BL-079`: stage samples before replacing canonical assets; roster edits transfer independently of portrait completion. Do not invent new CCR engine APIs.
...
- Out of this epic:
  ...
  - `BL-079` Operator Art Studio (manual review / cherry-pick regen / prompt-edit multi-sample / winner-only transfer in `kturner75/ccr-studio`). **`BL-065.3` stays bulk/unattended catalog regen**; the review studio is not this epic.
- Scope Buckets:
...
| BL-065.3 Curated catalog after trial | Proposed | After the trial looks clean, regen the curated catalog the same way — **bulk/unattended**. **Not done.** Do not fold into the prompt PR. Manual keep/regen review is `BL-079`, not this slice | Curated titles have xAI casts with the same PRIMARY/SECONDARY quality bar; #132 and `BL-064` remain separate; review studio stays `BL-079` |
- Acceptance Criteria:
...
- Distinct from `BL-079` (Operator Art Studio). `.3` is bulk/unattended catalog regen; `BL-079` is the local review / cherry-pick / multi-sample studio in `kturner75/ccr-studio`. Do not fold.
...
- 2026-08-24: Manual review / cherry-pick regen / prompt-edit multi-sample / winner-only transfer is `BL-079` (separate local app in `kturner75/ccr-studio`; originally captured as `BL-073` in #160). This epic’s `.3` remains bulk/unattended catalog regen. Do not fold the studio into `.3`.
...
- Distinct from `BL-079` (Operator Art Studio — local review/regen/transfer in `kturner75/ccr-studio`). Do **not** reuse `BL-073` (that id is the teacher capability API on `main`).
...
- Dependency Notes:
  ...
  - Distinct from `BL-079` (Operator Art Studio in `kturner75/ccr-studio`; originally captured as `BL-073` in #160). This id is the teacher capability API; do not reuse it for the studio.
- Risks:
...
- Dependency Notes:
  ...
  - Distinct from `BL-079` (Operator Art Studio in `kturner75/ccr-studio`). Studio uses this seam; do not invent a second generate API.
- Session Log:
...
- Dependency Notes:
  ...
  - Distinct from `BL-079` (Operator Art Studio). Studio keep/restore uses this upload; do not invent a second write-back API.
- Session Log:
...
### BL-079 - Operator Art Studio (local review / regen / transfer)
- Type: Feature / operator tool
- Priority: P2 (quality of catalog art; not a classroom blocker)
- Effort: L
- Status: Proposed
- Id note: Originally captured as **`BL-073`** in #160 while that PR was open. `main` later assigned `BL-073` to the teacher CREATE_CLASSROOM operator API (ccr-studio ST-009). Next free id is **`BL-079`**. Do **not** reuse `BL-072` / `BL-073`. Studio itself now lives in **`kturner75/ccr-studio`** (not a reader `/admin` route). This CCR capture does **not** invent new CCR engine APIs — use existing seams (`BL-074` portrait request/regen, `BL-075` upload, `CacheTransferRunner` + `sync_spaces.sh`).
- Problem: Character portraits from Imagine are really good; chapter illustrations are still hit-or-miss. Kevin (2026-08-24) wants a **manual review loop** so he can fine-tune images before they go to prod — review, override discovery types, cherry-pick regen one artifact, edit the Imagine prompt, request multiple samples, keep a winner, and transfer only accepted results.
- Hard constraint (Kevin, 2026-08-24 / 2026-08-27): this is an **entirely local, separate web app for him**. It now lives in **`kturner75/ccr-studio`**. Not part of the deployable CCR product. Not on classicchatreader.com. Not a `/admin` route in the reader. Do not put any of this UI in classic-chat-reader’s product jar, `index.html`, `teacher.html`, or prod deploy.
- Current code: Local CCR already generates and caches portraits/illustrations and runs discovery (`deployment.mode=local`, `generation.cache-only=false`). Prod is `generation.cache-only=true` and must stay that way — never kick Imagine/discovery on prod. Existing transfer: `CacheTransferRunner` `--feature portraits` carries roster identity; `--feature illustrations` promotes illustration metadata; `scripts/sync_spaces.sh` pushes binary assets. `CacheTransferRunner.exportPortraits` emits only existing **COMPLETED** characters with a portrait filename; `importPortraits` inserts/updates emitted rows and **never deletes** absent ones. Import conflict default is `skip` (old filename / prompt / `completed_at` stay). `scripts/publish_book_remote.sh` is a full-book pregen + remote promotion path — **do not use it** for this studio (Kevin: pregnerates remotely). CDN `?v=` (#158) cache-busts from asset-row version — a skipped metadata update leaves `?v=` unchanged even if Spaces overwrites the binary. Chat/call stays PRIMARY-only; chapter miner must not invent SECONDARY. `IllustrationService.restoreCachedIllustrationIfPresent` reconciles a chapter from its stable cached asset key — a regen that restamps siblings or clobbers neighbors via that path is a defect for this studio. Current multi-sample regen writes each candidate to the **stable cache key** and can destroy accepted art if N samples run before keep. Existing CCR HTTP seams for one-character generate/upload are `BL-074` / `BL-075` — do not invent a new engine API in this item.
- Current Direction:
- Separate local web app in **`kturner75/ccr-studio`**. Tracked here because it operates on CCR catalog assets. Do **not** invent new CCR engine APIs in this capture.
- Talks to **local** CCR only (`deployment.mode=local`, `generation.cache-only=false`). Prod stays cache-only.
- Kick off jobs for a **specific book**, **specific character**, or **specific chapter illustration** (not only whole-catalog pregen).
- Book picker (`BL-079.1`): catalog-sized set. Type-ahead autocomplete, results ordered by the visible title, keyboard/listbox semantics, explicit no-results. Local Kevin tool — not student UI and not a VPAT slice; AGENTS.md selection-control bar is the reason. Keep it short; do not invent a full a11y epic.
- Character discovery: start job, then **review / override type (PRIMARY vs SECONDARY) / delete** names. Do not re-enable the chapter miner as a roster inventor. PRIMARY-only chat/call stays the product rule.
- **Roster deletions must reach prod:** a name Kevin deletes in the studio must leave the production roster (and must not stay callable if it was PRIMARY). Today `exportPortraits`/`importPortraits` cannot do that. `BL-079.5` requires explicit deletion/tombstone or **reviewed-roster replacement**. We already hit leftover Moon/Mule/bees on prod for this reason. Cherry-pick keep-siblings still applies to *images*; roster review is the exception that may remove names.
- **Roster edits transfer independently of portrait completion:** PRIMARY/SECONDARY / identity changes must reach prod even when a portrait is pending, failed, or missing. Today `exportPortraits` only emits **COMPLETED** rows that have a filename, so a type override with no accepted portrait never leaves the machine. `BL-079.5` needs a roster-review transfer representation that is independent of portrait status and explicitly carries additions, type changes, and deletions.
- Portraits: review image, **keep or regen** until happy. Regen may **edit the AI image prompt**.
- Chapter illustrations: same keep/regen + prompt edit.
- **Cherry-pick regen (must-have):** regen one chapter illustration or one portrait (or one discovery name) while **keeping every other artifact** for that book. No whole-book wipe, no restamp of siblings, no `restoreCachedIllustrationIfPresent` clobber of neighbors.
- **Prompt edit + multi-sample (must-have):** edit the Imagine prompt and request **multiple samples** for the same artifact, then **choose the winner** and discard the rest. Keep/regen until happy still stands; N-way sample is the choose-best path.
- **Stage samples before replacing canonical assets (must-have):** N samples must use **uniquely keyed candidate storage**. Promote the winner to the canonical file + metadata **only after keep**. Current regen writes to the stable cache key as each candidate finishes, so a failed or abandoned review can destroy accepted art. Do not invent a new CCR generate API for this — studio-side candidate keys plus existing `BL-075` upload / keep, or an equivalent local staging path, is enough.
- Then **push/transfer** accepted / **winner-only** results to DO Spaces + prod DB. Local losers stay local or get deleted, never pushed.
- **Regenerated metadata must overwrite:** wrapping `CacheTransferRunner` defaults to `skip`, so prod keeps the old filename / prompt / `completed_at` and CDN `?v=` will not change even if `sync_spaces.sh` overwrites the binary. Require `--on-conflict overwrite` (or equivalent selective update) for accepted regenerated artifacts. Winner-only still; do not overwrite siblings.
- Auth: Kevin-on-his-machine only. No student/teacher accounts. No FERPA student data.
- Out of this epic:
  - `BL-065.3` bulk catalog regen (that stays bulk/unattended; this is the review studio).
  - Portrait-as-reference for illustration consistency (queued, not this first cut).
  - #132 name-identity dedupe (held). Do not touch #132.
  - `BL-064` spoiler blurbs.
  - `BL-078` assignment resume (originally `BL-072` in #159).
  - `BL-072` curated membership and `BL-073` teacher capability API (those ids collided after this capture).
  - Inventing new CCR engine APIs (`BL-074` / `BL-075` already cover per-character generate + upload).
  - Shipping any of this in the public jar / `index.html` / `teacher.html` / classicchatreader.com / a reader `/admin` route.
- Scope Buckets:
- Separate local studio app shell in `kturner75/ccr-studio` (type-ahead book picker + job status) that talks only to local CCR.
- Discovery review (PRIMARY/SECONDARY list, type override, delete; cherry-pick one name; no miner invent; deletions and type/identity edits transfer to prod even without a completed portrait).
- Portrait keep/regen with cherry-pick, prompt edit, N-way sample choose-winner, and staged candidate keys.
- Illustration keep/regen with the same cherry-pick + prompt-edit multi-sample + staged-candidate loop.
- Winner-only transfer to Spaces + prod DB (overwrite regenerated metadata; tombstone or reviewed-roster replacement for deleted names; roster edits independent of portrait completion; wrap existing scripts; no remote generate; no new CCR engine API).
- Work Tracker (suggested):
| Slice | Status | Scope | Done When |
| --- | --- | --- | --- |
| BL-079.1 Local studio app shell | Proposed | `kturner75/ccr-studio` (not CCR `/admin`). Book picker + job status. Talks to local CCR only. Book picker: type-ahead, title-ordered results, keyboard/listbox, explicit no-results (AGENTS.md selection-control bar; local Kevin tool, not student UI / not a VPAT slice) | Kevin can open the studio locally, type-ahead to a book, move the list with the keyboard, see no-results when nothing matches, and see job status — without any new UI on classicchatreader.com or in the product jar |
| BL-079.2 Discovery review | Proposed | List PRIMARY/SECONDARY; override type; delete; cherry-pick regen of one discovery name without wiping the rest of the roster; no miner invent. **Delete is the roster exception:** removing a name is allowed; cherry-pick keep-siblings still applies to *images*. Prod deletion + type/identity transfer is `BL-079.5` — a local delete or PRIMARY/SECONDARY change that never leaves prod is not done | Kevin can start a discovery job for one book, override type, delete a name, and regen one name while every other roster row stays; miner does not invent SECONDARY; the deleted name and type/identity edits are marked for transfer so they will not remain stale on prod |
| BL-079.3 Portrait keep/regen + prompt edit | Proposed | Keep or regen one portrait until happy. **Cherry-pick:** only that portrait changes; siblings stay. **Prompt edit + multi-sample:** edit the Imagine prompt, request N samples for the same portrait, choose the winner, discard the rest. **Stage samples:** uniquely keyed candidates; promote to the canonical file/metadata only after keep | Kevin can edit a portrait prompt, generate multiple samples without overwriting the accepted portrait until keep, pick a winner, and leave other portraits/illustrations untouched |
| BL-079.4 Illustration keep/regen + prompt edit | Proposed | Same keep/regen loop for one chapter illustration. **Cherry-pick:** no whole-book wipe, no sibling restamp, no `restoreCachedIllustrationIfPresent` clobber of neighbors. **Prompt edit + multi-sample:** edit prompt, request N samples, choose the winner, discard the rest. **Stage samples:** uniquely keyed candidates; promote to canonical only after keep | Kevin can edit one Frankenstein chapter-illustration prompt, generate multiple samples without destroying the accepted illustration, keep a winner, and leave every other chapter illustration untouched |
| BL-079.5 Transfer accepted assets to Spaces + prod DB | Proposed | Wrap existing transfer (`CacheTransferRunner` portraits/illustrations + `sync_spaces.sh`). Winner-only. Do **not** use `publish_book_remote.sh`. No remote generate. No new CCR engine API. **Deletions:** explicit deletion/tombstone or **reviewed-roster replacement** so a studio-deleted name leaves prod (export/import today never deletes absent rows; leftover Moon/Mule/bees). **Roster edits independent of portrait:** PRIMARY/SECONDARY / identity changes transfer even when the portrait is pending/failed/missing (`exportPortraits` today only emits COMPLETED rows with a filename). **Overwrite:** `--on-conflict overwrite` (or equivalent selective update) for accepted regenerated artifacts so filename / prompt / `completed_at` change and CDN `?v=` busts; do not overwrite siblings | Only accepted winners land on Spaces + prod DB with updated metadata; a name Kevin deleted locally is gone from prod and not callable; a PRIMARY/SECONDARY change without a completed portrait still reaches prod; local losers are never pushed; no full-book pregen; CDN `?v=` (#158) changes for the transferred winners |
- Acceptance Criteria:
- Kevin can review/regen one Frankenstein chapter illustration and one portrait locally, edit the prompt, keep a winner, and transfer only those to prod without a full-book pregen and without any new UI on classicchatreader.com.
- **Cherry-pick:** regen of one illustration, one portrait, or one discovery name keeps every other *image* artifact for that book. No whole-book wipe, no sibling restamp, no `restoreCachedIllustrationIfPresent` clobber of neighbors. Roster review may remove names; that deletion must still reach prod.
- **Prompt edit + multi-sample:** he can edit the Imagine prompt, request multiple samples for the same artifact, choose the winner, and discard the rest. Keep/regen until happy still works.
- **Stage samples:** N samples use uniquely keyed candidate storage. Canonical file + metadata update only after keep. Current stable-cache-key overwrite is not acceptable — a failed or abandoned N-sample run must not destroy accepted art.
- Winner-only transfer: accepted results go to Spaces + prod DB; local losers stay local or are deleted and are never pushed.
- **Deletions reach prod:** a name deleted in the studio is removed from the production roster (tombstone or reviewed-roster replacement). It does not stay on prod and is not callable if it was PRIMARY. Plain portrait upsert is not enough.
- **Roster edits reach prod without a completed portrait:** PRIMARY/SECONDARY / identity changes transfer even when the portrait is pending, failed, or missing. `exportPortraits` COMPLETED+filename-only is not enough.
- **Regenerated metadata overwrites:** accepted regenerated artifacts transfer with `--on-conflict overwrite` (or equivalent selective update) so prod filename / prompt / `completed_at` update and CDN `?v=` changes. Default `skip` is not acceptable for winners. Do not overwrite siblings.
- Book picker: type-ahead, title-ordered results, keyboard/listbox, explicit no-results. Local operator tool; not student UI; not a VPAT slice.
- Studio remains a separate local app in `kturner75/ccr-studio`. Prod stays `generation.cache-only=true`. No student/teacher accounts. No FERPA student data. Nothing ships in the public jar. No new CCR engine APIs are invented here.
- Dependency Notes:
- Distinct from `BL-065` / `BL-065.3` (bulk/unattended curated-catalog regen). This is the review studio, not that bulk pass. Cross-link both ways; do not fold.
- Distinct from #132 (name-identity dedupe). Held; do not touch and do not stack this capture on that PR.
- Distinct from `BL-064` (spoiler-safe blurbs). Reviewing art is not rewriting blurbs.
- Distinct from `BL-078` (assignment resume; originally `BL-072` in #159).
- Distinct from `BL-072` (curated membership) and `BL-073` (teacher CREATE_CLASSROOM API). Those ids collided after #160 captured this studio as `BL-073`.
- Distinct from `BL-074` / `BL-075` (existing CCR portrait request/regen + upload seams). The studio **uses** those; this epic does not invent a new engine API.
- Distinct from `BL-036` (book cover curation console — Discovery; in-app vs separate tool still open there). This epic is portraits / illustrations / discovery review, and the surface is **required** to be `kturner75/ccr-studio`, not a reader `/admin`.
- Complements existing transfer (`CacheTransferRunner` portraits feature carries roster identity; `sync_spaces.sh`) and #158 CDN `?v=` cache-bust. Do not invent a new remote generate path. Do not treat default `skip` or insert-only portrait import as sufficient.
- Risks:
- Implementing this as `/admin` or pages in `index.html` / `teacher.html` would put operator Imagine controls on the public product.
- Using `publish_book_remote.sh` or any prod-side pregen would kick Imagine/discovery on prod (cache-only violation) and transfer more than the winners.
- A regen path that wipes the book, restamps siblings, or lets `restoreCachedIllustrationIfPresent` restore/clobber neighbors would destroy accepted art.
- Writing N samples to the stable cache key replaces the accepted image as candidates finish, so a failed/abandoned review destroys accepted art.
- Single-sample-only regen without prompt edit leaves Kevin with the same hit-or-miss illustration loop.
- Transfer that only upserts portraits leaves studio-deleted PRIMARY names callable on prod (the leftover Moon/Mule/bees path).
- Transfer that only emits COMPLETED+filename rows drops PRIMARY/SECONDARY / identity edits when the portrait is pending, failed, or missing.
- Wrapping `CacheTransferRunner` with default `skip` leaves old `completed_at`, so CDN `?v=` does not change after Spaces overwrites the binary.
- Inventing a new CCR `/admin` or engine API in this item duplicates `BL-074` / `BL-075` and pulls studio into the public jar.
- Session Log:
- 2026-08-24: Captured from Kevin. Imagine portraits are really good; chapter illustrations still hit-or-miss. Wants a manual review loop before prod. **Hard constraint:** entirely local, separate web app — not the CCR jar, not classicchatreader.com, not `/admin`. Local CCR only; prod cache-only. Specific book / character / chapter-illustration jobs. Discovery review (override PRIMARY/SECONDARY, delete; no miner invent). Portraits + illustrations: keep/regen + prompt edit, then winner-only transfer (`sync_spaces.sh` / portraits feature; not `publish_book_remote.sh`). #158 `?v=` already cache-busts. Out: `BL-065.3` bulk regen, portrait-as-reference, #132, `BL-064`, assignment resume, shipping in the public jar. Originally id **BL-073**. Docs only; **Proposed**.
