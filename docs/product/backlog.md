...
- Current code: `CharacterPrefetchService` routes through `reasoningLlmProvider` (xAI after `BL-056.5` / PR #112). Discovery prompts (prefetch + chapter extraction) now require a named-people-only roster, a tight PRIMARY / sane SECONDARY split, and first-appearance-only stored descriptions (`BL-064` generation rule; chapter `up-to` gating unchanged). `CharacterRosterNameFilter` rejects mass nouns / animals / objects / places / collectives / Moon-Mule class / LLM leftovers on **both** prefetch parse and extraction persist. `DELETE /api/characters/book/{bookId}` clears `books.character_prefetch_completed` so a later pregen re-runs PRIMARY prefetch (ADMIN-gated in `SensitiveApiRequestMatcher`). Pregen image defaults: portraits already `character.portrait.generation.provider=xai` / `grok-imagine-image`; covers default `book-cover.generation.provider=xai`; illustrations gained the same provider seam (`illustration.generation.provider=xai`, Comfy opt-in). Prod is `generation.cache-only=true`; characters must be pregenerated locally and transferred. Jobs: `POST /api/pregen/jobs/book/{bookId}`, `POST /api/pregen/jobs/gutenberg/{gutenbergId}`. Existing stored casts are still local-Ollama until Kevin runs the two-book trial (Gutenberg **174** + **84**). Do not treat this code slice as `.3` catalog-done.
- Current Direction:
...
| BL-065.2 Trial book (Frankenstein 84 + Dorian Gray 174) | In Progress (code; regen not run) | Prompt + filter + DELETE latch + Imagine defaults so the trial regen can be clean. Do **not** run pregen/Imagine in CI or prod in this slice | After Kevin’s local regen, Dorian Gray / Frankenstein no longer show junk secondaries (“bees”, “The Moon”, “The Mule”, “Elizabeth Lavenza (again)”) or spoiler blurbs; no duplicate/glitch name cards |
