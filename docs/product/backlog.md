...
- 2026-08-26: `BL-075` HTTP write-back in progress this PR: portrait + illustration multipart `PUT`, cover provenance (`studio` + prompts), PNG-only, cache-only 409. No Imagine enqueue. No Flyway.
...
| Slice | Status | Scope | Done When |
| --- | --- | --- | --- |
| BL-075.1 Portrait upload | In Progress | Multipart write of PNG + `source` + prompt metadata onto the character’s live portrait | Studio keep/restore of `prt-*` is HTTP-only |
| BL-075.2 Illustration upload | In Progress | Same for a chapter illustration | Studio keep/restore of `ill-*` is HTTP-only |
| BL-075.3 Cover PUT provenance | In Progress | Cover upload accepts `source` + prompt fields; studio keep is not stamped `manual_upload` | Kept covers carry the Imagine prompt and a studio source |
- Acceptance Criteria:
