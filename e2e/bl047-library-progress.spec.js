...
test('Continue Reading restores the saved resume point before leaving wrap-up', async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem('reader_bookActivity', JSON.stringify({
      'book-1': {
        chapterCount: 3,
  ...
  expect(activity.lastChapterIndex).toBe(2);
  expect(activity.lastPage).not.toBeUndefined();
});

test('paragraph and chapter shortcuts persist after dismissing Open wrap-up', async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem('reader_bookActivity', JSON.stringify({
      'book-1': {
        chapterCount: 3,
